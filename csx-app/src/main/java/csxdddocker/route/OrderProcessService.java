...
//import csxdddocker.model.OrderStockTraderListDTO;
...
public class OrderProcessService {
    ...
    public void processOrder(ArrayList<Order> allOrders) {
        //* Filter valid active lists of allOrders,contraBuyOrders,contraSellOrders
        allOrders = allOrders.stream().filter(ord ->(ord.getIsactive() && ord.getSize()>0 && ord.getPrice().compareTo(new BigDecimal(0)) > 0)).collect(Collectors.toCollection(ArrayList::new));
        ArrayList<Order> contraBuyOrders = allOrders.stream().filter(ord ->(ord.getIsactive() && ord.getSide().trim().equalsIgnoreCase("BUY") && ord.getSize()>0 && ord.getPrice().compareTo(new BigDecimal(0)) > 0)).collect(Collectors.toCollection(ArrayList::new));
        ArrayList<Order> contraSellOrders = allOrders.stream().filter(ord ->(ord.getIsactive() && ord.getSide().trim().equalsIgnoreCase("SELL") && ord.getSize()>0  && ord.getPrice().compareTo(new BigDecimal(0)) > 0)).collect(Collectors.toCollection(ArrayList::new));
        //*
